...
trend_df = pd.read_csv(
    DATA_DIR / f"{coin_name}_trend_log_returns_2019-2025.csv", parse_dates=["date"]
)

price_df = pd.read_csv(DATA_DIR / f"{coin_name}_price_mcap.csv", parse_dates=["date"])
...
for idx, (start_date, end_date) in enumerate(zip(keydates[:-1], keydates[1:])):
    ...
    df_merged = df_merged.dropna(
        subset=transaction_col + ["trend_log_return", "fear_greed_log_return"]
    )
    ...
    analyser.run_granger(max_lag=7)
