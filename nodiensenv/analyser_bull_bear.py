...
class EnhancedTechnicalAnalyser:
    def __init__(self, ohlcv_df, sentiment_analyzer=None):
...
class EnhancedRegimeSpecificPredictor:
    ...
    def create_features(self):
        ...
        print("Calculating enhanced technical indicators with sentiment...")
        # Calculate enhanced technical indicators
        ta = EnhancedTechnicalAnalyser(btc, sentiment_analyzer)
        ...
        print(f"Enhanced feature set created: {self.feature_df.shape}")
    ...
    def select_robust_features(self):
        ...
        base_features = [
            ...
            # Google Trends LOG RETURN features
            "search_log_return",
            "search_log_return_ma_7",
            "search_log_return_ma_30",
            "search_log_return_momentum",
            "search_acceleration",
            "high_search_momentum",
            "low_search_momentum",
            "extreme_search_spike",
            "extreme_search_drop",
            "search_price_momentum_corr",
            "search_volatility",
            "search_volatility_regime",
            # Combined sentiment
            "combined_sentiment",
            "search_log_return_normalized",
        ]

        # Filter available features
        available_features = [f for f in base_features if f in self.feature_df.columns]

        # Feature selection using statistical tests
        if len(available_features) > 12:
            selector = SelectKBest(score_func=f_classif, k=12)
            selector.fit(self.feature_df[available_features], self.feature_df["target"])
            selected_features = [
                available_features[i] for i in selector.get_support(indices=True)
            ]
            self.unified_features = selected_features
        else:
            self.unified_features = available_features

        print(
            f"Selected {len(self.unified_features)} features including log return sentiment: {self.unified_features}"
        )
        return self
    ...
    def train_ensemble_models(self):
        ...
        self.ensemble_model.fit(X_unified, y_unified)

        ensemble_score = accuracy_score(
            y_unified, self.ensemble_model.predict(X_unified)
        )
    ...
    def train_regime_models(self):
        ...
        self.unified_model = XGBClassifier(**self.model_params)
        ...
        print(f"Unified model training accuracy: {unified_score:.4f}")
        print(f"Unified model training precision: {unified_precision:.4f}")
        print(f"Unified model training recall: {unified_recall:.4f}")
        print(f"Unified model training F1 score: {unified_f1:.4f}")
    ...
    def walk_forward_validation(
        ...
        if results["unified_model"]:
            for model_name in ["unified_model", "ensemble_model"]:
                if results[model_name]:
                    accuracy = sum(results[model_name]) / len(results[model_name])
                    print(
                        f"{model_name}: {accuracy:.4f} ({len(results[model_name])} predictions)"
                    )

            # Print regime distribution
            from collections import Counter

            regime_counts = Counter(results["regimes"])
            print(f"\nRegime Distribution:")
            for regime, count in regime_counts.items():
                print(
                    f"  {regime}: {count} predictions ({count/len(results['regimes'])*100:.1f}%)"
                )
    ...
    def get_feature_importance_analysis(self):
        ...
        # Unified model importance
        if self.unified_model is not None:
            for feature, importance in zip(
                self.unified_features, self.unified_model.feature_importances_
            ):
        ...
        # Ensemble model importance (XGBoost component)
        if self.ensemble_model is not None:
            xgb_estimator = self.ensemble_model.named_estimators_["xgb"]
            for feature, importance in zip(
                self.unified_features, xgb_estimator.feature_importances_
            ):
                importance_data.append(
                    {
                        "model": "Ensemble_XGB",
        ...
        if not importance_df.empty:
            ...
            for i, (feature, importance) in enumerate(
                avg_importance.head(10).items(), 1
            ):
            ...
            # Plot 2: Ensemble XGBoost component
            plt.subplot(2, 1, 2)
            ...
            try:
                ...
                if len(X_regime) >= 20:  # Minimum for CV
                    cv_scores = cross_val_score(
                        model, X_regime, y_regime, cv=3, scoring="accuracy"
                    )
                ...
                train_pred = model.predict(X_regime)
                train_acc = (train_pred == y_regime).mean()
                ...
                if hasattr(locals(), "cv_mean") and train_acc - cv_mean > 0.15:
                    print(
                        f"    ⚠️  WARNING: Possible overfitting (train-CV gap: {train_acc - cv_mean:.1%})"
                    )
        ...
        for regime in ["bull", "bear"]:
            if regime in self.regime_models and self.regime_models[regime] is not None:
                ...
                if hasattr(model, "feature_importances_"):
                    ...
                    # Categorize features (customize based on your feature names)
                    categories = {
                        "Price_Position": [
                            "price_vs_sma",
                            "bb_position",
